...
and destination kind, and rejects drift in the documentation and Python-wheel
copies.
...
DOC_YAML_MIRROR = ROOT / "docs/design/observability-v8/config-v8-observability-reference.yaml"
...
def _outputs(schema_bytes: bytes) -> dict[Path, bytes]:
    ...
    return {
        ...
        DOC_YAML_MIRROR: yaml_bytes,
        PYTHON_DATA_DIR / SCHEMA_PATH.name: schema_bytes,
        PYTHON_DATA_DIR / CANONICAL_YAML.name: yaml_bytes,
        PYTHON_DATA_DIR / CANONICAL_MARKDOWN.name: markdown_bytes,
    }
...
def _check(outputs: Mapping[Path, bytes]) -> bool:
    ok = True
    package_data_staged = PYTHON_DATA_DIR.is_dir()
    for path, expected in outputs.items():
        if path.parent == PYTHON_DATA_DIR and not package_data_staged:
            # The entire _data tree is a gitignored build-staging area. A
            # source checkout need not have it until `make _bundle-data` or
            # `make dist-cli`; when present, however, every copy must match.
            continue
        try:
