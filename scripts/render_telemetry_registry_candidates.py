...
@dataclasses.dataclass(frozen=True, slots=True)
class _PublicViewRenderEntry:
    """One fully validated public schema and all byte-identical staged targets."""
...
@dataclasses.dataclass(frozen=True, slots=True)
class _PublicViewRenderPlan:
    """Immutable public-view authority admitted at a render boundary."""

    generator: str
    registry_version: int
    baseline_epoch: str
    authority_sha256: str
    views: tuple[_PublicViewRenderEntry, ...]
...
PUBLIC_VIEW_CANDIDATE_PREFIX: Final = f"{GENERATED_PREFIX}/public-views"
...
_PUBLIC_VIEW_LOGICAL_OUTPUT_PATHS: Final = tuple(
    sorted(
        {
            *(output_path for _view_id, output_path in _PUBLIC_VIEW_IDENTITIES),
            *(target for targets in _PUBLIC_VIEW_MIRRORS.values() for target in targets),
        },
        key=str.encode,
    )
)
PUBLIC_VIEW_CANDIDATE_OUTPUT_PATHS: Final = tuple(
    f"{PUBLIC_VIEW_CANDIDATE_PREFIX}/{output_path}" for output_path in _PUBLIC_VIEW_LOGICAL_OUTPUT_PATHS
)
if (
    len(PUBLIC_VIEW_CANDIDATE_OUTPUT_PATHS) != 26
    or len(set(PUBLIC_VIEW_CANDIDATE_OUTPUT_PATHS)) != 26
    or len({_candidate_path_identity(path) for path in PUBLIC_VIEW_CANDIDATE_OUTPUT_PATHS}) != 26
):  # pragma: no cover - static closed inventory invariant
    raise RuntimeError("public-view candidate output inventory is not portable")
_STATIC_CANDIDATE_OUTPUT_PATHS: Final = (
    *_BASE_CANDIDATE_OUTPUT_PATHS,
    *PUBLIC_VIEW_CANDIDATE_OUTPUT_PATHS,
)
...
def _validate_public_views(
    path_value: FrozenJSON,
    value: FrozenJSON,
    registry_version: int,
) -> _PublicViewRenderPlan:
    """Validate public-view authority and return an immutable render plan."""
    ...
    render_entries: list[_PublicViewRenderEntry] = []
    ...
    for view_index, (raw_view, (expected_id, expected_path)) in enumerate(
        ...
        if (
            not document_bytes.startswith(b"{")
            or not document_bytes.endswith(b"}")
            or JSON_OWNERSHIP_MARKER in document_bytes
        ):
            raise CandidateRenderError("materialized public view document is not an unmarked compact object")
        try:
        ...
        render_entries.append(
            _PublicViewRenderEntry(
                id=view_id,
                output_path=output_path,
                target_paths=tuple(
                    f"{PUBLIC_VIEW_CANDIDATE_PREFIX}/{target}" for target in (output_path, *expected_mirrors)
                ),
                dialect=dialect,
                schema_id=schema_id,
                baseline_document_canonical_json=document_bytes,
            )
        )
    ...
    planned_paths = tuple(path for entry in render_entries for path in entry.target_paths)
    if (
        len(planned_paths) != len(PUBLIC_VIEW_CANDIDATE_OUTPUT_PATHS)
        or frozenset(planned_paths) != frozenset(PUBLIC_VIEW_CANDIDATE_OUTPUT_PATHS)
        or len({_candidate_path_identity(path) for path in planned_paths}) != len(planned_paths)
    ):
        raise CandidateRenderError("materialized public view target inventory is invalid")
    return _PublicViewRenderPlan(
        generator=_string(marker["generator"], "public views marker generator"),
        registry_version=registry_version,
        baseline_epoch=_string(marker["baseline_epoch"], "public views marker epoch"),
        authority_sha256=authority_sha256,
        views=tuple(render_entries),
    )
...
_PUBLIC_VIEW_MARKER_KEYS: Final = frozenset({"generator", "registry_version", "public_view_id", "baseline_epoch"})


def _render_public_view_payload(document_bytes: bytes, marker: Mapping[str, Any]) -> bytes:
    """Splice one closed authority marker into validated compact object bytes."""

    if (
        not isinstance(marker, Mapping)
        or frozenset(marker) != _PUBLIC_VIEW_MARKER_KEYS
        or marker.get("generator") != "scripts/generate_telemetry_registry.py"
        or type(marker.get("registry_version")) is not int
        or marker["registry_version"] < 1
        or not isinstance(marker.get("public_view_id"), str)
        or not marker["public_view_id"]
        or marker.get("baseline_epoch") != "public-schemas-v7-marker-only-v1"
    ):
        raise CandidateRenderError("public view generated marker is invalid")
    if (
        not isinstance(document_bytes, bytes)
        or len(document_bytes) < 2
        or not document_bytes.startswith(b"{")
        or not document_bytes.endswith(b"}")
    ):
        raise CandidateRenderError("public view baseline document is not a compact object")
    if JSON_OWNERSHIP_MARKER in document_bytes:
        raise CandidateRenderError("public view baseline document already carries generated authority")
    marker_member = JSON_OWNERSHIP_MARKER + b":" + _canonical_json_bytes(marker)
    object_members = document_bytes[1:-1]
    return b"{" + marker_member + (b"," + object_members if object_members else b"") + b"}\n"


def _render_public_view_candidate_artifacts(
    plan: _PublicViewRenderPlan,
) -> Mapping[str, CandidateArtifact]:
    """Render the closed staged public-view inventory without filesystem I/O."""

    if (
        not isinstance(plan, _PublicViewRenderPlan)
        or plan.generator != "scripts/generate_telemetry_registry.py"
        or plan.registry_version < 1
        or plan.baseline_epoch != "public-schemas-v7-marker-only-v1"
        or plan.authority_sha256 != _PUBLIC_VIEW_AUTHORITY_SHA256
        or tuple((entry.id, entry.output_path) for entry in plan.views) != _PUBLIC_VIEW_IDENTITIES
        or any(
            entry.target_paths
            != tuple(
                f"{PUBLIC_VIEW_CANDIDATE_PREFIX}/{target}"
                for target in (entry.output_path, *_PUBLIC_VIEW_MIRRORS.get(entry.output_path, ()))
            )
            for entry in plan.views
        )
    ):
        raise CandidateRenderError("public view render plan authority is invalid")
    planned_paths = tuple(path for entry in plan.views for path in entry.target_paths)
    if (
        len(planned_paths) != 26
        or frozenset(planned_paths) != frozenset(PUBLIC_VIEW_CANDIDATE_OUTPUT_PATHS)
        or _preflight_candidate_output_paths(planned_paths) != planned_paths
    ):
        raise CandidateRenderError("public view render plan paths are invalid")
    ...
    for entry in plan.views:
        ...
        payload = _render_public_view_payload(entry.baseline_document_canonical_json, marker)
        for path in entry.target_paths:
            _add_candidate_artifact(
                ...
                CandidateArtifact(
                    ...
                    ownership_marker=JSON_OWNERSHIP_MARKER,
                ),
            )
    if len(artifacts) != 26:
        raise CandidateRenderError("public view render inventory is incomplete")
...
def render_candidate_artifacts_from_index(model: CandidateRenderIndex) -> Mapping[str, CandidateArtifact]:
    ...
    public_view_plan = _validate_public_views(
        model.fields["public_views_path"],
        model.fields["public_views"],
    ...
    for artifact in _render_public_view_candidate_artifacts(public_view_plan).values():
