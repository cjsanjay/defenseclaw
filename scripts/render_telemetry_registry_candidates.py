...
class _PublicViewRenderEntry:
    """One fully validated public schema and all byte-identical live targets."""
...
def _normalized_candidate_path(raw: str) -> str:
    ...
    if not under_generated_root and raw not in PUBLIC_VIEW_OUTPUT_PATHS:
        raise CandidateRenderError("candidate output path is outside the generated root and public-view allowlist")
...
PUBLIC_VIEW_OUTPUT_PATHS: Final = _PUBLIC_VIEW_LOGICAL_OUTPUT_PATHS
if (
    len(PUBLIC_VIEW_OUTPUT_PATHS) != 26
    or len(set(PUBLIC_VIEW_OUTPUT_PATHS)) != 26
    or len({_candidate_path_identity(path) for path in PUBLIC_VIEW_OUTPUT_PATHS}) != 26
):  # pragma: no cover - static closed inventory invariant
    raise RuntimeError("public-view live output inventory is not portable")
_STATIC_CANDIDATE_OUTPUT_PATHS: Final = (
    *_BASE_CANDIDATE_OUTPUT_PATHS,
    *PUBLIC_VIEW_OUTPUT_PATHS,
)
...
_PUBLIC_VIEW_AUTHORITY_SHA256: Final = "67d9feb8e5d7afe7f6a4157cf68570f27802e01d4bd89e00a64f1f9d869fb4fd"
...
def _validate_public_views(
    ...
    for view_index, (raw_view, (expected_id, expected_path)) in enumerate(
        ...
        if (
            _string(view["authority"], f"{context} authority") != PUBLIC_VIEW_GENERATED_AUTHORITY
        ...
        render_entries.append(
            _PublicViewRenderEntry(
                ...
                target_paths=(output_path, *expected_mirrors),
                dialect=dialect,
                schema_id=schema_id,
    ...
    if (
        len(planned_paths) != len(PUBLIC_VIEW_OUTPUT_PATHS)
        or frozenset(planned_paths) != frozenset(PUBLIC_VIEW_OUTPUT_PATHS)
...
def _render_public_view_artifacts(
    plan: _PublicViewRenderPlan,
) -> Mapping[str, CandidateArtifact]:
    """Render the closed live public-view inventory without filesystem I/O."""
    ...
    if (
        ...
        or any(
            entry.target_paths != (entry.output_path, *_PUBLIC_VIEW_MIRRORS.get(entry.output_path, ()))
...
def render_candidate_artifacts_from_index(model: CandidateRenderIndex) -> Mapping[str, CandidateArtifact]:
    ...
    for artifact in _render_public_view_artifacts(public_view_plan).values():
        _add_candidate_artifact(artifacts, artifact)
    return _preflight_candidate_artifacts(artifacts)
...
__all__ = [
    "CANDIDATE_AUTHORITY",
    "PUBLIC_VIEW_GENERATED_AUTHORITY",
    "PUBLIC_VIEW_OUTPUT_PATHS",
    "CandidateArtifact",
    "CandidateAttribute",
    "CandidateDomain",
