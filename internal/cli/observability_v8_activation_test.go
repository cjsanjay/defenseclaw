...
func TestPrepareObservabilityV8StartupCompilesAndProjectsLocalPaths(t *testing.T) {
	...
	c := &config.Config{
		...
		DataDir:        directory,
		AuditDB:        "legacy-audit.db",
		JudgeBodiesDB:  "legacy-judge.db",
	}
...
func TestLegacyConfigLoaderHandsSchemaV8ToCanonicalActivation(t *testing.T) {
	directory := t.TempDir()
	configPath := filepath.Join(directory, "config.yaml")
	raw := "config_version: 8\n" +
		"data_dir: " + directory + "\n" +
	...
	loaded, err := config.LoadFromFileWithRuntimeMigration(configPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	startup, err := prepareObservabilityV8Startup(loaded)
	if err != nil {
		t.Fatal(err)
	}
...
func TestBootstrapConfiguredObservabilityRuntimePreservesV7NoOp(t *testing.T) {
