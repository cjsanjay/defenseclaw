...
// V8GenerationSpanPipeline selects exactly one delivery mode for a named
// destination. Canonical and Legacy are deliberately an XOR: a canonical
// destination never falls back to its raw SDK processor path.
type V8GenerationSpanPipeline struct {
	Destination string
	Canonical   V8CanonicalSpanConsumer
	Legacy      sdktrace.SpanProcessor
}
...
func cleanupV8SpanPipelines(pipelines []V8GenerationSpanPipeline, timeout time.Duration) {
	...
	v8BoundedPrepareCleanup(timeout, func(ctx context.Context) {
		...
		for index := len(pipelines) - 1; index >= 0; index-- {
			...
			if cleanupV8PipelineChild(pipeline.Legacy, seen) {
				_ = v8CallProcessorContext(pipeline.Legacy.Shutdown, ctx)
			}
		}
	})
}
...
func (processor *v8CompositeSpanProcessor) OnStart(ctx context.Context, span sdktrace.ReadWriteSpan) {
	if !processor.beginCallback() {
		return
	}
	defer processor.endCallback()
	for index := range processor.pipelines {
		legacy := processor.pipelines[index].Legacy
		if legacy != nil {
			v8CallLegacyOnStart(legacy, ctx, span)
		}
	}
}
...
func (processor *v8CompositeSpanProcessor) OnEnd(span sdktrace.ReadOnlySpan) {
	...
	for index := range processor.pipelines {
		...
		switch {
		case pipeline.Canonical != nil && hasCanonical:
			v8CallCanonicalEnqueue(pipeline.Canonical, canonical)
		case pipeline.Legacy != nil:
...
func (processor *v8CompositeSpanProcessor) ForceFlush(ctx context.Context) error {
	...
	for index := range processor.pipelines {
		...
		if pipeline.Canonical != nil {
			...
			err = v8CallProcessorContext(pipeline.Legacy.ForceFlush, ctx)
		}
...
func (processor *v8CompositeSpanProcessor) runShutdown() {
	...
	for index := len(processor.pipelines) - 1; index >= 0; index-- {
		...
		if pipeline.Canonical != nil {
			...
			err = v8CallProcessorContext(pipeline.Legacy.Shutdown, ctx)
		}
		failed = failed || err != nil
	}
	if failed {
...
func validV8SpanPipelines(pipelines []V8GenerationSpanPipeline) bool {
	...
	for _, pipeline := range pipelines {
		if !observability.IsStableToken(pipeline.Destination) || len(pipeline.Destination) > v8MaxDestinationNameBytes ||
			(pipeline.Canonical == nil) == (pipeline.Legacy == nil) {
			return false
		}
		...
		var child any = pipeline.Canonical
		if pipeline.Legacy != nil {
			child = pipeline.Legacy
		}
		identity, ok := identityOfV8PipelineChild(child)
		if !ok {
			return false
		}
...
func v8CallLegacyOnStart(processor sdktrace.SpanProcessor, ctx context.Context, span sdktrace.ReadWriteSpan) {
	defer func() { _ = recover() }()
	processor.OnStart(ctx, span)
}

func v8CallLegacyOnEnd(processor sdktrace.SpanProcessor, span sdktrace.ReadOnlySpan) {
	defer func() { _ = recover() }()
...
func v8CallProcessorContext(call func(context.Context) error, ctx context.Context) (err error) {
	defer func() {
		if recover() != nil {
			err = errors.New("telemetry: legacy span processor failed")
		}
	}()
	return call(ctx)
}
