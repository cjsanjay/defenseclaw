...
type LocalEventAppender interface {
	GraphDigest() string
	AppendContext(context.Context, observability.Record, redaction.Projection) error
}

// Projector is the immutable redaction-engine seam. Production passes one
// *redaction.Engine; the interface keeps destination-failure isolation
// independently testable without exposing projection constructors.
type Projector interface {
	Project(observability.Record, redaction.Profile) (redaction.Projection, redaction.SafeReport, error)
}
...
type ProjectionFailureRecord struct {
	...
	RedactionProfile redaction.ProfileName
...
func (pipeline *LocalLogPipeline) process(
	...
	localProfile, ok := pipeline.catalog.Resolve(redaction.ProfileName(local.RedactionProfile))
	if !ok {
...
func boundedProjectionFailure(err error) OptionalFailureCode {
	var projectionError *redaction.ProjectionError
	if !errors.As(err, &projectionError) {
		return OptionalFailureProjection
	}
	switch projectionError.Code {
	case redaction.ProjectionFailureClassification:
		return OptionalFailureClassification
	case redaction.ProjectionFailureMetricClass:
		return OptionalFailureMetricClass
	case redaction.ProjectionFailureContext:
		return OptionalFailureContext
	case redaction.ProjectionFailureOutputLimit:
		return OptionalFailureOutputLimit
	case redaction.ProjectionFailureSerialization:
