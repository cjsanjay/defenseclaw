...
func TestInboundImportedLogConstructsEveryGeneratedContextAsOrdinary(t *testing.T) {
	...
	if got := len(catalog.snapshot.contexts); got != 93 {
		t.Fatalf("import contexts = %d, want 93", got)
	}
	...
	if ids.count.Load() != 93 {
		t.Fatalf("occurrence IDs = %d, want 93", ids.count.Load())
	}
...
func TestInboundImportedLogConstructsEveryGeneratedLogTargetWithExactSource(t *testing.T) {
	catalog, err := LoadInboundCatalog()
	if err != nil {
		t.Fatal(err)
	}
	...
	if count != 96 {
		t.Fatalf("imported log targets = %d, want 96", count)
	}
...
func TestInboundImportedLogUsesAllFourSealedStructuredBindings(t *testing.T) {
	...
	outputMessages.Items[0].FinishReason = "mutated"
