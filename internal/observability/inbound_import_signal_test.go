...
func TestInboundImportedTraceConstructsEveryGeneratedTarget(t *testing.T) {
	...
	if count != 31 || ids.count.Load() != uint64(count) {
		t.Fatalf("trace targets/IDs = %d/%d, want 31/31", count, ids.count.Load())
	}
}

func TestInboundImportedNativeMetricConstructsEveryReversibleTarget(t *testing.T) {
	catalog, err := LoadInboundCatalog()
	if err != nil {
		t.Fatal(err)
	}
	...
	if imports != 104 || derives != 14 || ids.count.Load() != uint64(imports+derives) {
		t.Fatalf("metric import/derive/IDs = %d/%d/%d, want 104/14/118", imports, derives, ids.count.Load())
	}
}
...
func TestInboundDerivedMetricRejectsUnsealedSourceFactsBeforeOccurrence(t *testing.T) {
	...
	codex := mustInboundImportTarget(t, catalog,
		"otlp.codex.response_completed.v1.log.model.response.metric.gen_ai.client.operation.duration")
	...
	crossUse := []struct {
		...
		{name: "codex-as-gauge", target: codex, source: NewInboundMetricGaugeSource("s")},
