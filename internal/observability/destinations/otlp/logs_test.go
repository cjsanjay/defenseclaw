...
func TestLogAdapterSnapshotsGenerationResourceAndIsolatesSiblings(t *testing.T) {
	...
	if got := protoLogResourceValue(first.resource, "service.instance.id"); got != "generation-one" {
		t.Fatalf("first adapter observed source mutation: %q", got)
	}
	if got := protoLogResourceValue(second.resource, "defenseclaw.custom.team.name"); got != "security-platform" {
		t.Fatalf("second adapter observed source mutation: %q", got)
	}
	first.resource.Attributes[0].Value.Value = nil
	first.resource.DroppedAttributesCount = 99
	if got := protoLogResourceValue(second.resource, "service.name"); got != "defenseclaw" ||
		second.resource.DroppedAttributesCount != 7 || second.resourceURL != snapshot.SchemaURL {
		t.Fatalf("sibling resource was not isolated: service=%q dropped=%d schema=%q", got, second.resource.DroppedAttributesCount, second.resourceURL)
	}
}
