...
func TestGeneratedRecordScrapeCoversEveryCatalogShapeAliasesBoundsAndCumulativeSemantics(t *testing.T) {
	...
	// The production bridge keeps not-yet-migrated producers on the same
	// endpoint while generated producers use the sibling exporter.
	harness.provider.RecordScan(t.Context(), "skill", "skill", "clean", 11, nil, "codex")
	// Overlap is permitted during producer-by-producer cutover. The same family
	// and label tuple must remain scrapeable rather than producing a duplicate
	// collector error.
	harness.provider.RecordConnectorHookInvocation(t.Context(), "codex", "prompt", "ok", "allow", 2)
	harness.provider.RecordActivity(t.Context(), "reload", "config", "operator", 1)
	harness.provider.RecordAgentDiscoverySignal(t.Context(), "codex", true, true, true, "ok")
	harness.provider.RecordSinkCircuitState(t.Context(), "otlp", "primary", 2)
	...
	for _, want := range []string{
		...
		`defenseclaw_scan_count_total{scanner="skill",target_type="skill",verdict="clean"} 1`,
		`defenseclaw_activity_total{action="reload",actor="operator",target_type="config"} 6`,
		`defenseclaw_judge_persist_batch_size_bucket{le="8"} 1`,
		`defenseclaw_connector_hook_latency_milliseconds_bucket{connector="codex",event_type="prompt",reason="allow",result="ok",le="10"} 2`,
		`defenseclaw_connector_hook_latency_milliseconds_sum{connector="codex",event_type="prompt",reason="allow",result="ok"} 9.5`,
		`defenseclaw_connector_hook_latency_milliseconds_count{connector="codex",event_type="prompt",reason="allow",result="ok"} 2`,
		...
		`defenseclaw_audit_sink_circuit_state{sink_kind="otlp",sink_name="primary"} 4`,
	} {
...
func TestGeneratedPrometheusMaterializesAndScrapesAllExact131Contracts(t *testing.T) {
	...
	descriptors, err := telemetry.V8MetricDescriptorCatalog()
	if err != nil || len(descriptors) != 131 {
	...
	if got := strings.Count(body, "# TYPE "); got != 131 || len(sink.instruments) != 131 {
		t.Fatalf("scraped families=%d instruments=%d", got, len(sink.instruments))
	}
...
func TestGeneratedPrometheusCardinalityLimitUsesOneOverflowSeries(t *testing.T) {
	...
	for index := 0; index < 2_050; index++ {
		harness.provider.RecordAgentDiscoverySignal(
			t.Context(), "legacy-connector-"+strconv.Itoa(index), true, true, true, "ok",
		)
	}
	for index := 0; index < 2_050; index++ {
		harness.record(t, generatedMetricBuilt(builder.BuildMetricDefenseClawAgentDiscoveryInstalled(
...
func TestGeneratedPrometheusCollectionGatePrecedesDestinationConstruction(t *testing.T) {
	...
	if prepared.MetricPipelines[0].Projection != telemetry.V8MetricProjectionLocal || len(prepared.MetricPipelines[0].SelectedFamilies) != 131 ||
