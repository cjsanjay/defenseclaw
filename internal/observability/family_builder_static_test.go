...
func TestGeneratedTelemetryFamilyBuilderPublicAPIMatchesCompilerAllowlist(t *testing.T) {
	sources := observabilityPackageSources(t)
	encoded, found, err := generatedFamilyBuilderAllowlist(sources)
	if err != nil {
		t.Fatal(err)
	}
	if !found {
	...
	assertFamilyBuilderStaticSourceAPIJSONWithSources(t, encoded, sources)
}
...
func TestGeneratedTelemetryExplicitOverlayCandidate(t *testing.T) {
	...
	if !found {
		t.Fatal("generated telemetry overlay has no FamilyBuilder allowlist")
	}
	assertFamilyBuilderStaticAPIJSONWithSources(t, encoded, sources)
	if err := validateGeneratedTelemetrySourceSafety(sources); err != nil {
		t.Fatal(err)
	}
}

func assertFamilyBuilderStaticAPIJSON(t *testing.T, encoded string) {
	t.Helper()
	contracts, err := decodeFamilyBuilderMethodContracts(encoded)
	if err != nil {
		t.Fatalf("decode generated FamilyBuilder API allowlist: %v", err)
	}
...
func assertFamilyBuilderStaticAPIJSONWithSources(
	t *testing.T,
	encoded string,
...
func assertFamilyBuilderStaticSourceAPIJSONWithSources(
	t *testing.T,
	encoded string,
	sources []parsedPackageSource,
) {
...
func decodeFamilyBuilderMethodContracts(encoded string) ([]familyBuilderMethodStaticContract, error) {
	decoder := json.NewDecoder(strings.NewReader(encoded))
	decoder.DisallowUnknownFields()
	var contracts []familyBuilderMethodStaticContract
	if err := decoder.Decode(&contracts); err != nil {
		return nil, err
	}
	if err := decoder.Decode(&struct{}{}); err != io.EOF {
		return nil, fmt.Errorf("allowlist has trailing JSON")
	}
	seen := make(map[string]struct{}, len(contracts))
	for index, contract := range contracts {
		if !token.IsExported(contract.Name) || contract.ReceiverType != "FamilyBuilder" ||
			!contract.ReceiverPointer || !contract.InputNamedStruct || contract.InputType == "" ||
			len(contract.ResultTypes) != 2 || contract.ResultTypes[0] != "Record" ||
			contract.ResultTypes[1] != "error" || contract.Variadic {
			return nil, fmt.Errorf("allowlist method %d has a noncanonical signature", index)
		}
		if _, duplicate := seen[contract.Name]; duplicate {
			return nil, fmt.Errorf("allowlist method %s is duplicated", contract.Name)
		}
		seen[contract.Name] = struct{}{}
		if index > 0 && contracts[index-1].Name >= contract.Name {
			return nil, fmt.Errorf("allowlist methods are not strictly ordered")
		}
	}
...
func generatedFamilyBuilderAllowlist(sources []parsedPackageSource) (string, bool, error) {
	var encoded string
	found := false
	for _, source := range sources {
		for _, declaration := range source.file.Decls {
			...
			for _, raw := range general.Specs {
				...
				for index, name := range spec.Names {
					if name.Name != "generatedFamilyBuilderMethodContractsJSON" {
						continue
					}
					if found || index >= len(spec.Values) {
						return "", false, fmt.Errorf("generated FamilyBuilder allowlist is duplicated or valueless")
					}
					literal, ok := spec.Values[index].(*ast.BasicLit)
					if !ok || literal.Kind != token.STRING {
						return "", false, fmt.Errorf("generated FamilyBuilder allowlist is not a string constant")
					}
					value, unquoteErr := strconv.Unquote(literal.Value)
					if unquoteErr != nil {
						return "", false, fmt.Errorf("decode generated FamilyBuilder allowlist: %w", unquoteErr)
					}
