...
func TestLegacyV7PureHelpersIgnoreGlobalState(t *testing.T) {
	scenarios := []struct {
		...
		{name: "disable environment", disableEnv: "1"},
		{name: "disable runtime override", disableOverride: true},
		{name: "all bypasses", revealEnv: "1", disableEnv: "1", disableOverride: true},
	}

	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			t.Setenv(revealEnvVar, scenario.revealEnv)
			t.Setenv(disableEnvVar, scenario.disableEnv)
			SetDisableAll(scenario.disableOverride)
...
func TestLegacyV7SinkWrappersDelegateAtGlobalBoundary(t *testing.T) {
	...
	SetDisableAll(true)
	if got := ForSinkReason("RULE:hello world"); got != "RULE:hello world" {
		t.Fatalf("global boundary did not preserve v7 disable behavior: got %q", got)
	}
	if got := LegacyV7Reason("RULE:hello world"); got != "RULE:<redacted len=11 sha=b94d27b9>" {
		t.Fatalf("global boundary leaked into pure helper: got %q", got)
	}
}
