...
func TestOTLPInboundClassifierGeneratedPositiveNegativeAndSingleFaultCorpus(t *testing.T) {
	classifier := mustOTLPInboundClassifierV8(t)
	matches := allInboundCatalogMatches(t, classifier.catalog)
	if got, want := len(matches), 237; got != want {
		t.Fatalf("generated matches = %d, want %d", got, want)
	}
