...
func TestLocalJudgeGeneratedSpanAllCallPaths(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			withCapturedEvents(t)
			runtime, capture := newProxyGeneratedTraceRuntime(t)
			reader := sdkmetric.NewManualReader()
			legacyExporter := tracetest.NewInMemoryExporter()
			provider, err := telemetry.NewProviderForTraceTest(reader, legacyExporter)
			if err != nil {
				t.Fatal(err)
			}
			...
			judge.SetTelemetryProvider(provider)
			...
			for _, legacy := range legacyExporter.GetSpans() {
				if legacy.Name == "defenseclaw.guardrail.judge" {
					t.Fatalf("v8 %s call duplicated the legacy judge span", test.kind)
				}
			}
			if count := judgeLatencyMetricCount(t, reader); count != 1 {
				t.Fatalf("judge latency sample count=%d, want 1", count)
			}
		})
	}
}

func TestLocalJudgeLegacySpanRemainsV7Only(t *testing.T) {
	withCapturedEvents(t)
	reader := sdkmetric.NewManualReader()
	...
	if len(spans) != 1 || spans[0].Name != "defenseclaw.guardrail.judge" {
		t.Fatalf("legacy spans=%v", spans)
	}
	legacyAttributes := make(map[string]any, len(spans[0].Attributes))
	for _, attribute := range spans[0].Attributes {
		legacyAttributes[string(attribute.Key)] = attribute.Value.AsInterface()
	}
	if legacyAttributes["judge.kind"] != "injection" || legacyAttributes["judge.verdict"] != "allow" {
		t.Fatalf("legacy judge attributes=%v", legacyAttributes)
	}
	if _, canonical := legacyAttributes["defenseclaw.span.family"]; canonical {
		t.Fatal("v7 legacy span was mislabeled as canonical v8")
	}
	if count := judgeLatencyMetricCount(t, reader); count != 1 {
		t.Fatalf("judge latency sample count=%d, want 1", count)
	}
...
func TestLocalJudgeAdjudicationLegacySpanRemainsV7Only(t *testing.T) {
	withCapturedEvents(t)
	reader := sdkmetric.NewManualReader()
	exporter := tracetest.NewInMemoryExporter()
	provider, err := telemetry.NewProviderForTraceTest(reader, exporter)
	...
	judge := &LLMJudge{
		...
		provider: &mockLLMProvider{response: &ChatResponse{
			Model: "openai/gpt-judge", Choices: []ChatChoice{{
				Message:      &ChatMessage{Role: "assistant", Content: `{"findings":[],"overall_threat":false,"severity":"NONE"}`},
	...
	if verdict.JudgeFailed || verdict.Action != "allow" {
		t.Fatalf("legacy adjudication verdict=%+v", verdict)
	}

	spans := exporter.GetSpans()
	if len(spans) != 1 || spans[0].Name != "defenseclaw.guardrail.judge" {
		t.Fatalf("legacy adjudication spans=%v", spans)
	}
	attributes := make(map[string]any, len(spans[0].Attributes))
	for _, attribute := range spans[0].Attributes {
		attributes[string(attribute.Key)] = attribute.Value.AsInterface()
	}
	if attributes["judge.kind"] != "adjudicate_injection" || attributes["judge.verdict"] != "allow" {
		t.Fatalf("legacy adjudication attributes=%v", attributes)
	}
	if count := judgeLatencyMetricCount(t, reader); count != 1 {
		t.Fatalf("judge latency sample count=%d, want 1", count)
	}
...
func TestLocalJudgeLegacyFailureStatusParity(t *testing.T) {
	tests := []struct {
		name        string
		...
		{
			name: "provider", provider: &mockLLMProvider{err: errors.New("provider unavailable")},
			status: "Error", description: "provider unavailable",
		},
		{
			name: "empty", provider: &mockLLMProvider{response: &ChatResponse{}},
			status: "Ok",
		},
		{
			name: "parse", provider: &mockLLMProvider{response: &ChatResponse{
				Choices: []ChatChoice{{Message: &ChatMessage{Content: "not-json"}}},
			}},
			status: "Error", description: "parse-failed",
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			judge := &LLMJudge{
				cfg:   &config.JudgeConfig{Enabled: true, Injection: true},
				model: "openai/gpt-judge", provider: test.provider, rp: &guardrail.RulePack{},
			}
			judge.SetTelemetryProvider(provider)
			judge.runInjectionJudge(t.Context(), strings.Repeat("i", 40))
			...
			spans := exporter.GetSpans()
			if len(spans) != 1 {
				t.Fatalf("legacy failure spans=%v", spans)
			}
			if spans[0].Status.Code.String() != test.status ||
				spans[0].Status.Description != test.description {
				t.Fatalf("legacy failure spans/status=%v/%s/%q want=%s/%q", spans, spans[0].Status.Code, spans[0].Status.Description, test.status, test.description)
			}
			if count := judgeLatencyMetricCount(t, reader); count != 1 {
				t.Fatalf("judge latency sample count=%d, want 1", count)
			}
...
func TestLocalJudgeGeneratedSpanFailureFacts(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// Generated tracing must complete even when the independent legacy
...
func TestLocalJudgeAdjudicationGeneratedFailureFacts(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, legacy := range legacyExporter.GetSpans() {
				if legacy.Name == "defenseclaw.guardrail.judge" {
					t.Fatal("v8 adjudication failure duplicated legacy judge span")
				}
			}
			if count := judgeLatencyMetricCount(t, reader); count != 1 {
				t.Fatalf("judge latency sample count=%d, want 1", count)
...
func TestJudgeGeneratedSamplingDropPropagatesStartedContext(t *testing.T) {
	...
	propagated, operation, _ := judge.startJudgeTrace(
		original, "injection", "prompt", 1024,
		[]ChatMessage{{Role: "user", Content: "content"}}, time.Now().UTC(),
	)
	if propagated.Value(contextKey{}) != "sampled-drop" || operation == nil || !operation.v8 || operation.generated != nil {
	...
	failed, _, _ := judge.startJudgeTrace(
		original, "injection", "prompt", 1024,
		[]ChatMessage{{Role: "user", Content: "content"}}, time.Now().UTC(),
	)
...
func TestJudgeGeneratedContentUsesBoundedGeneratedFields(t *testing.T) {
	...
	output, outputBytes, outputState, outputEvents, reported := judgeV8OutputMessages(
		outputContent, "stop", time.Now().UTC(),
	)
...
func TestJudgeGeneratedRuntimeBindingFollowsSidecarLifecycle(t *testing.T) {
	...
	hookJudge.SetTelemetryProvider(provider)
	proxyJudge := &LLMJudge{}
	...
	_, hookRuntime, hookAuthority := hookJudge.judgeTelemetrySnapshot()
	_, proxyRuntime, proxyAuthority := proxyJudge.judgeTelemetrySnapshot()
	...
	for _, span := range legacyExporter.GetSpans() {
		if span.Name == "defenseclaw.guardrail.judge" {
			t.Fatal("detached v8 authority resurrected a legacy judge span")
		}
	}
}

func judgeLatencyMetricCount(t *testing.T, reader *sdkmetric.ManualReader) uint64 {
	t.Helper()
	...
	for _, scope := range resource.ScopeMetrics {
		for _, metric := range scope.Metrics {
			if metric.Name != "defenseclaw.guardrail.judge.latency" {
				continue
			}
			histogram, ok := metric.Data.(metricdata.Histogram[float64])
			if !ok {
				t.Fatalf("judge latency metric type=%T", metric.Data)
			}
