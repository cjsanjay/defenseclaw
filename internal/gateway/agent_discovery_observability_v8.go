...
func (a *APIServer) agentDiscoveryV8Authority() (sidecarRuntimeEmitter, bool) {
	if a == nil {
		return nil, false
	}
	emitter := a.observabilityV8RuntimeEmitter()
	cfg := a.runtimeConfigSnapshot()
	return emitter, emitter != nil || cfg != nil && cfg.ConfigVersion == 8
}

func (a *APIServer) emitAgentDiscoverySummary(ctx context.Context, summary agentDiscoveryTelemetrySummary) {
	emitter, authoritative := a.agentDiscoveryV8Authority()
	if !authoritative {
		if a != nil && a.otel != nil {
			a.otel.RecordAgentDiscovery(ctx, summary.source, summary.cacheHit, summary.result, float64(summary.durationMs), summary.agentsTotal, summary.installedTotal)
			a.otel.EmitAgentDiscoverySummaryLog(ctx, summary.source, summary.cacheHit, summary.result, float64(summary.durationMs), summary.agentsTotal, summary.installedTotal)
		}
		return
	}
...
func (a *APIServer) emitAgentDiscoverySignal(ctx context.Context, reportSource, connector string, signal agentDiscoverySignal, probeStatus string) {
	...
	if !authoritative {
		if a != nil && a.otel != nil {
			a.otel.RecordAgentDiscoverySignal(ctx, connector, signal.Installed, signal.HasConfig, signal.HasBinary, probeStatus)
			a.otel.EmitAgentDiscoverySignalLog(ctx, connector, signal.Installed, signal.HasConfig, signal.HasBinary, probeStatus)
		}
		return
	}
	if emitter == nil || ctx == nil {
...
func (a *APIServer) emitAgentDiscoveryError(ctx context.Context, reportSource, connector, reason string) {
	...
	if !authoritative {
		if a != nil && a.otel != nil {
			a.otel.RecordAgentDiscoveryError(ctx, connector, reason)
		}
		return
	}
	if emitter == nil || ctx == nil {
