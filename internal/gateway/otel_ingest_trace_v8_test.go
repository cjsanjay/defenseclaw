...
func compileOTLPTracePlan(
	...
	if eventNames == nil {
		destination.Send = &config.ObservabilityV8SendSource{
			...
			Buckets:          []observability.Bucket{observability.BucketTelemetryIngest},
...
func TestOTLPIngestGeneratedTraceAcceptsJSONAndProtobufWithoutRawPayload(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if receive.Name() != "POST telemetry" || normalize.Name() != "telemetry.normalize logs" ||
				!ok || parent != receive.SpanID() || receive.StatusCode() != codes.Ok || normalize.StatusCode() != codes.Ok {
			...
			if len(events) != 1 || events[0].eventName != "telemetry.batch.accepted" || strings.Contains(events[0].payload, test.secret) {
...
func TestOTLPIngestGeneratedTraceSamplingCollectionRoutingAndLoopSafety(t *testing.T) {
	...
	t.Run("self export produces no recursive records", func(t *testing.T) {
		...
		body := `{"resourceSpans":[{"resource":{"attributes":[
			...
			{"key":"defenseclaw.span.family","value":{"stringValue":"span.telemetry.receive"}},
