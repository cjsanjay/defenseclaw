...
func CompileObservabilityV8(source *ObservabilityV8Source) (*ObservabilityV8Plan, error) {
	...
	if len(source.Destinations) > ObservabilityV8MaxDestinations {
		return nil, fmt.Errorf("observability.destinations: got %d, maximum is %d", len(source.Destinations), ObservabilityV8MaxDestinations)
	}
...
func compileObservabilityV8Destination(
	...
	if err := validateObservabilityV8Headers(source.Headers, path+".headers"); err != nil {
...
func validateObservabilityV8ResolvedOTLPEndpoints(
	...
	for _, signal := range selected {
		...
		if strings.TrimSpace(endpoint) == "" {
			return fmt.Errorf("%s.signal_overrides.%s.endpoint: selected signal has no resolved endpoint", path, signal)
		}
...
func validateObservabilityV8Headers(source map[string]ObservabilityV8HeaderValue, path string) error {
	for name, value := range source {
		if strings.TrimSpace(name) == "" || len(name) > 256 {
			return fmt.Errorf("%s: header name must contain 1 through 256 bytes", path)
		}
