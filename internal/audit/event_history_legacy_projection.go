...
// contextWithLegacyEventProjection carries the already-sanitized v7 view into
// EventHistoryWriter. The same canonical runtime transaction can therefore
// populate additive v8 columns without changing the legacy API columns or
// inserting a second audit_events row.
...
func legacyEventProjectionFromContext(
	...
	if event.ID != record.RecordID() || !event.Timestamp.UTC().Equal(record.Timestamp()) ||
		...
		event.TraceID != correlation.TraceID || event.RequestID != correlation.RequestID ||
	...
	case observability.EventName(observability.TelemetryEventConfigChangeApplied),
		...
		observability.EventName(observability.TelemetryEventAssetDiscovered):
	default:
