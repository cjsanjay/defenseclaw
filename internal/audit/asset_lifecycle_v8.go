...
// LogAssetDiscoveredCtx preserves the v7 install-detected Event projection
// while supplying the source-backed identity facts required by the generated
// asset.discovered family. It is required when Event.Target is a path rather
// than the stable asset identifier.
...
func (l *Logger) emitAssetLifecycleV8(
	...
	if emitter == nil || mapping.CanonicalEvent == "" {
		return auditV8Unhandled, nil
	}
	if err := validateAssetLifecycleInput(input); err != nil {
		// Existing installations may contain asset names that predate the v8
		// stable-identifier grammar. They remain observable through the exact v7
		// audit/OTel path instead of being silently dropped by the watcher.
		return auditV8Unhandled, nil
	}
	if ctx == nil {
