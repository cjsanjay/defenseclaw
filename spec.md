...
last_verified_commit: cbd652a84
...
| `P5-WP02` | `IN_PROGRESS` | root + generation/public-view/structural subagents + Claude review | `P5-WP01` | Generated bundle/catalog/docs/constants/builders/field classes/selector registries/fixtures | Commits `852b19195`, `5bb317cf6`, `606abc337`, and `f9de2f438` preserve the complete immutable IR, materialize deterministic P-067 group resolution, and bind exact outcomes for 87 logs/25 spans while removing metric envelope outcomes. Commits `e17f64ad8`, `e309dffc3`, and `b9b4fd335` define, implement, and pin the lossless twenty-one-schema pre-cutover baseline with generated-authority recursion protection. Commit `df0d16382` adds P-069's closed structural authority, and `99f8d9949` adds the recursively immutable typed materialized view/digest, six trace derivations, coherent uint32 family versions, and the private builder kernel. Commits `33090c49d` and `bab42f800` establish and harden the manifest-last multi-root generated-output transaction; `0d4dd8964`, `883e38284`, and `bad7486d6` add and close the in-memory portable schema/catalog/docs/example/OTLP candidate renderer, value/event arms, and full-set path preflight. Spec commits `dfba24401`, `94b3ae966`, and `efa5b6e0e` close the P-070 source grammar while preserving open GenAI values and exact identity/bound semantics. Commit `cbd652a84` implements the exact eleven-rule mandatory catalog and explicit/exact-base example builder facts. Commit `f99a40092` compiles the four pinned OTel GenAI inputs into all 21 structured types, four bindings, exact property dispositions, open-provider extensions, privacy/numeric/Blob encoding metadata, and complete P-069/P-070 candidate trust boundaries; it also makes upstream refresh serialized, compare-and-swap published, full-reference validated, and rollback/commit-state explicit. Commit `1a023d4f1` closes compiler/candidate parity for typed enums, portable full-match patterns, recursive/byte constraints, normalizer catalogs, direct/resolved provenance, and portable output paths. Commit `6eb29d997` resolves the Go const/type collision without changing the reviewed 1,773-row inventory: the 21 structured-type and 17 arm rows are emitted once as typed declarations and retain their stable IDs privately. Commits `f4eb4f234` and `d57a47d74` pin the closed override grammar and reproducible digest/golden contract. Commit `bcb6a0faa` compiles the exact 22-kind/1,773-row symbol table and `dc9c8340b` binds the candidate trust boundary to its d897 digest and all source identities without retokenization or golden I/O; `aa36ebe45` refreshes the materialized manifest. Commit `d9e301ef4` makes the exact seven internal Go outputs an all-or-none desired/prior transaction inventory. Commit `4ecce4044` closes the remaining generated-API authority: eleven derived trace values, selected-family floor semantics, enriched descriptor and independent digest contracts, exact scoped selectors/layouts/signatures/conversions, structured sealed shapes, and compiler-owned 893/282/212/386 file assignment. P5-WP02 remains open: source/compiler materialization, rendering the exact seven generated Go outputs, typed public views, generated-Go builders/constants, and atomic authority cutover are not yet implemented. |
...
| `C-0023` | 2026-07-04 | generated-output transaction boundary | `P5-WP02..06` | P-030, P-046, P-061, P-069 | Candidate and cutover renderers publish exact schema and Go output allowlists through one manifest-last logical transaction. Ownership is content-addressed by manifest digest, marker, and mode; the seven internal Go paths are an exact all-or-none desired and prior inventory; journals and private staging are owner-only, bounded, canonical, crash-recoverable, and identity-bound. All reads, mutations, cleanup, and linked-worktree resolution fail closed through descriptor-relative no-follow operations. Python 3.10-3.14 use the same security semantics, including a descriptor-relative recursive cleanup fallback. | complete |
| `C-0024` | 2026-07-04 | generated-builder source grammar | `P5-WP02..06` | P-069, P-070 | Keep P-069 as the sole canonical record authority and add only the missing generation facts through one closed P-070 grammar: dynamic mandatory rules, structured types/bindings, deterministic Go symbols, and explicit example builder contexts. Commits `dfba24401`, `94b3ae966`, and `efa5b6e0e` close that normative grammar; `cbd652a84` materializes the exact mandatory-rule catalog and example builder contexts into the immutable view and candidate renderer. The transaction boundary and portable candidate renderer are implemented and hardened through `33090c49d`, `0d4dd8964`, `883e38284`, `bad7486d6`, `bab42f800`, and `d9e301ef4`; `1a023d4f1` closes candidate constraint/provenance/path parity, `6eb29d997` assigns each reviewed Go symbol one compiler-owned declaration form, and `bcb6a0faa`/`dc9c8340b` compile and trust-bind the reproducible 1,773-row table without changing its reviewed source inventory. Commit `4ecce4044` specifies the compiler-owned enriched descriptor and Go API/file plan, exact derived values and selected-family floor semantics, and independent candidate digest needed to render mechanically. Source/compiler materialization, rendering the exact seven generated Go outputs, public views, and authority cutover remain open. Compatibility-only/removed identities still require no builder, and current runtime/public-schema authority cannot switch until every listed source blocker and parity gate passes. | in progress |
