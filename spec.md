...
last_verified_commit: 1a0dd1f48
...
| Ready queue | Wire committed OTLP transports into immutable runtime generations, close partial-success semantics, add native Prometheus, then run the P3 E2E gate |
| Blocked | None |
...
| `P3-WP02` | `IN_PROGRESS` | root + OTel subagent | `P2-GATE` | OTLP log/trace/metric routing, projection, sampling, inbound normalization | Guarded bounded HTTP/gRPC log/trace/metric transports, generation-owned provider, sampling, resource truth, retry, partial counters, and exact canary substrate are committed; graph assembly and inbound v8 normalization remain |
...
| `P3-WP04` | `IN_PROGRESS` | root + Galileo subagent | `P3-WP02` | Galileo projection, delivery funnel, partial success, exact canary | Commit `9269b80c5` implements the already-redacted `galileo-rich-v2` compatibility projection; OTLP delivery funnel, generic partial-success accounting, and live exact canary remain |
| `P3-WP05` | `IN_PROGRESS` | root + local-observability subagent | `P3-WP02..03` | `local-observability-v1`, Collector pipeline, PR #412 query inventory | Static consumer-profile/query inventory and safe live-stack validation are active; existing 14-dashboard/313-panel and 33 focused gates remain green |
