...
last_verified_commit: 99f8d9949
...
| `P5-WP02` | `IN_PROGRESS` | root + generation/public-view/structural subagents + Claude review | `P5-WP01` | Generated bundle/catalog/docs/constants/builders/field classes/selector registries/fixtures | Commits `852b19195`, `5bb317cf6`, `606abc337`, and `f9de2f438` preserve the complete immutable IR, materialize deterministic P-067 group resolution, and bind exact outcomes for 87 logs/25 spans while removing metric envelope outcomes. Commits `e17f64ad8`, `e309dffc3`, and `b9b4fd335` define, implement, and pin the lossless twenty-one-schema pre-cutover baseline with generated-authority recursion protection. Commit `df0d16382` adds P-069's closed structural authority. Commit `99f8d9949` adds the recursively immutable typed materialized view and digest, six total trace derivations, coherent uint32 family versions, the private log/trace/metric builder kernel, and a manifest-last multi-root generated-output transaction verified across Python 3.10-3.14. P-070 now closes the remaining authored grammar for mandatory-rule facts, structured bindings, Go symbols, example builder contexts, the enriched candidate render index, and the exact seven-file all-or-nothing candidate set. Candidate bundle/catalog/generated-Go renderers, fixtures, typed public-view dispositions, and later atomic authority cutover remain. |
...
| `C-0024` | 2026-07-04 | generated-builder source grammar | `P5-WP02..06` | P-069, P-070 | Keep P-069 as the sole canonical record authority and add only the missing generation facts through one closed P-070 grammar: dynamic mandatory rules, structured types/bindings, deterministic Go symbols, and explicit example builder contexts. The compiler must materialize those sources into one immutable enriched candidate index consumed by every renderer. The exact seven generated Go files are one candidate-only all-or-nothing set; compatibility-only/removed identities have no builder, and current runtime/public-schema authority cannot switch until every listed source blocker and parity gate passes. | in progress |
