...
last_verified_commit: 4fadc4a6c
...
| `P5-WP02` | `IN_PROGRESS` | root + generation/public-view/structural subagents + Claude review | `P5-WP01` | Generated bundle/catalog/docs/constants/builders/field classes/selector registries/fixtures | Commits `852b19195` through `4200ad043` establish the lossless immutable registry IR, generated API/fixture plans, portable candidate renderer, exact seven-file Go renderer, structural/privacy/outcome/mandatory contracts, typed builders, and reproducible symbol/output authority. Commits `a8361ad02`, `753580e8b`, `c86068d03`, and `4fdadf297` publish the seven Go plus thirty portable artifacts through the manifest-last transaction with the exact static API and shared baseline reader. Commits `4fadc4a6c` and `8630abb16` add exact first-use and payload-free repeat adoption for the reviewed 26 public-schema/mirror/embed paths. Commits `d651c03a4` and `793473271` compile the complete 21-view/756-field/94-dynamic-scope/39-reference public contract, independently bind its render authority, harden module identity, and close every Claude finding while leaving current public authority unchanged. P5-WP02 now remains open only for rendering the marker-only public views, executing the legacy parity corpus, widening manifest ownership to the exact 26-path allowlist, and performing the one atomic adoption/consumer cutover after byte, semantic, `$id`/`$ref`, mirror, embed, and installed-package parity. |
...
| `C-0021` | 2026-07-03 | structural registry authority | `P5-WP02..03` | P-058, P-061, P-069, P-070 | Make one closed typed registry contract authoritative for the canonical envelope, correlation, provenance, signal payload arms, trace resource/scope/status/events/links, metric samples, privacy classes, conditions, lifecycle phase/code values, and canonical-to-OTLP representations. The source/compiler/runtime foundation and immutable materialized view are verified through `99f8d9949`; generated schemas, builders, catalogs, fixtures, and projections must consume that same view before this decision is complete. | in progress |
...
| `C-0024` | 2026-07-04 | generated-builder source grammar | `P5-WP02..06` | P-069, P-070 | Keep P-069 as the sole canonical record authority and add only the missing generation facts through one closed P-070 grammar: dynamic mandatory rules, structured types/bindings, deterministic Go symbols, and explicit example builder contexts. Commits `dfba24401` through `4200ad043` close and materialize that grammar, the mandatory-rule catalog, candidate trust boundary, reproducible 1,773-row symbol table, enriched API/file plan, and exact seven-file renderer. Commits `a8361ad02` through `4fdadf297` publish the seven Go and thirty portable outputs; `4fadc4a6c`/`8630abb16` provide exact first/repeat public adoption; and `d651c03a4`/`793473271` complete the independently pinned typed public-view compiler boundary. Compatibility-only/removed identities still require no builder. Marker-only public rendering, executable legacy parity, exact 26-path manifest widening, and the atomic public/consumer cutover remain open, so current runtime/public-schema authority cannot switch yet. | in progress |
