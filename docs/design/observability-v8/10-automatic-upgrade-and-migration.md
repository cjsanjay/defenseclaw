...
4. Back up the exact config source and any installed DefenseClaw-owned
...
- Preserve legacy redacted/unredacted intent for both local and optional
  ...
  broadening an upgraded installation.
...
- Never resolve a secret into YAML or diagnostic output.
...
- Candidate construction or validation failure leaves the original source bytes
  untouched.
- A partial multi-file activation restores both files from the backup.
