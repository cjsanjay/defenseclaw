...
| P-076 | Treat OpenClaw EventRouter lifecycle frames as bounded agent-run observations until a version-gated adapter retains `sessionId`, current/root/parent identity, explicit session kind, parent/root session topology, depth, and terminal classification. The only permissible partial successor is the outcome-free `log.agent.run.observed` occurrence family; it neither impersonates PR #403 lifecycle nor retains a trace/generation lease across deliveries. | Preserves the real `runId`, per-run sequence, millisecond timestamps, literal start/end/error observation, and session correlation while preventing false session/subagent transitions, ID conflation, retry-as-failure, replay assumptions, and reload-blocking cross-delivery handles. |
