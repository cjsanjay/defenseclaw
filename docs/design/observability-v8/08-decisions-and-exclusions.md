...
| P-058 | Fix canonical record envelope version and bucket catalog version at integer `1`; use an immutable, bounded, deterministically serialized generic record with an exact log/trace `body` versus metric `instrument_data` union, exact correlation/provenance objects, and complete field classification. P2 owns this generic substrate and current classified-log adapter, while P5-WP02 exclusively owns generated detailed family builders and schemas. | Gives the router one safe identity and serialization boundary now without duplicating or pre-empting the generated telemetry-family API. |
