...
| P-070 | Add one closed generated-builder source contract for dynamic mandatory rules, structured value types/bindings, stable Go symbols, and explicit example builder contexts; materialize it with P-069 into one immutable enriched `CandidateRenderIndex`, and accept the exact seven generated Go files only as an all-or-nothing candidate set. Compatibility-only/removed identities have no builder, public inputs expose no map/`any` or catalog authority, and candidate files do not activate current public-schema/runtime consumers. | Prevents renderers from inferring occurrence facts, mandatory state, nested structured shapes, or language names; closes the P-061 dynamic-key gap; makes builder/bundle/fixture parity executable without weakening P-069 or turning a partial generated set into a second authority. |
