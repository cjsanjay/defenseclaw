...
| P-071 | Add one closed `registry.yaml` `inbound_bindings` catalog to the P-069/P-070 materialized authority. Authored binding classes expand at compile time into one exact generated binding per eligible target. Generated discriminators, self-echo recognizers, typed mappings, private non-floor import contexts, fixtures, and the inbound compatibility manifest are the only accepted-record path; zero/ambiguous matches and arbitrary unknown fields have no generic fallback. | Makes bucket/family/classification authority reviewable and generated while preventing runtime wildcards, hand-coded name heuristics, sender-selected buckets/floor state, and decoded raw pass-through. |
