...
def validate_config() -> ValidationResult:
    ...
    if gc is not None:
        ...
        if gc.scanner_mode not in ("local", "remote", "both"):
            res.errors.append(f"guardrail.scanner_mode must be 'local', 'remote', or 'both' (got '{gc.scanner_mode}')")
    ...
    if gw is not None:
        ...
        if str(reload_mode).strip().lower() not in ("hot", "restart"):
            res.errors.append(f"gateway.config_reload.mode must be 'hot' or 'restart' (got '{reload_mode}')")
