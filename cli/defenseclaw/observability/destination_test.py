...
class SocketProbeTransport:
    ...
    def handshake(self, target: _Target, *, timeout: float) -> None:
        ...
        address = _resolve_allowed_address(target, self._resolver, timeout)
        sock = self._open_socket(target, address, timeout)
...
def _parse_target(
    ...
    *,
    ...
    if (
        ...
        raise DestinationTestError("invalid_destination", "the destination endpoint is invalid")
    if parsed.fragment or parsed.query and protocol == "grpc":
...
def _resolve_allowed_address(target: _Target, resolver: Resolver, timeout: float) -> str:
